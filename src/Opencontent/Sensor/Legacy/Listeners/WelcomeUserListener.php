...
class WelcomeUserListener extends AbstractListener
{
    private $repository;
    ...
    {
        if ($param instanceof SensorEvent && $param->identifier == 'on_generate_user') {
            $user = $param->user;
            ...
            if (eZINI::instance('mbpaex.ini')->hasVariable("mbpaexSettings", "ForgotPasswordHashLifeTime")) {
                $forgotPasswdHashLifeTime = eZINI::instance('mbpaex.ini')->variable("mbpaexSettings", "ForgotPasswordHashLifeTime");
                $tpl->setVariable('hash_key_lifetime', date("d/m/Y H:i", time() + $forgotPasswdHashLifeTime));
            } else {
                $tpl->setVariable('hash_key_lifetime', false);
            }
            $tpl->setVariable('generate_password_link', "/userpaex/forgotpassword/{$hashKey}/");
            $templatePath = 'design:sensor/mail/welcome_user.tpl';
