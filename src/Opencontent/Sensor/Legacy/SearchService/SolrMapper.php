...
class SolrMapper
{
    ...
    /**
     * @see PostService::setUserPostAware()
     * @return void
    ...
    private function setPostUserUnaware()
    {
        ...
        $this->repository->getMessageService()->loadPostTimelineItems($userUnawarePost);
        $this->repository->getMessageService()->loadPostResponses($userUnawarePost);
        $this->repository->getParticipantService()->loadPostParticipants($userUnawarePost);

        $builder = new PostBuilder(
            $this->repository,
            $this->repository->getPostService()->getContentObject($userUnawarePost),
     ...
     */
    private function getFilteredTimelines($filter)
    {
        if (!isset($this->inMemoryValues[$filter]) && $this->post->timelineItems instanceof TimelineItemCollection){
            $this->inMemoryValues[$filter] = $this->post->timelineItems->getByType($filter);
        }

        return $this->inMemoryValues[$filter];
    }

    /**
     * @return array
     */
    public function mapExecutionTimesAndAssignements()
    {
        $data = array();
        ...
        $read = $this->getFilteredTimelines('read')->first();
        ...
        $assigned = $this->getFilteredTimelines('assigned')->first();
        ...
        $fix = $this->getFilteredTimelines('fixed')->last();
        ...
        $close = $this->getFilteredTimelines('closed')->last();
        ...
        if ($this->post->timelineItems instanceof TimelineItemCollection) {
            $data['sensor_is_read_i'] = $this->getFilteredTimelines('read')->count();
            $data['sensor_is_assigned_i'] = $this->getFilteredTimelines('assigned')->count();
            $data['sensor_is_fixed_i'] = $this->getFilteredTimelines('fixed')->count();
            $data['sensor_is_closed_i'] = $this->getFilteredTimelines('closed')->count();
        }
        ...
        foreach ($this->getFilteredTimelines('assigned')->messages as $message) {
            $lastOwnerUserId = 0;
            $lastOwnerGroupId = 0;
     ...
     * @return array
     */
    public function mapParticipants()
    {
        $data = array();
        ...
        if ($this->post->participants instanceof ParticipantCollection) {
            ...
            foreach ($this->post->participants->participants as $participant) {
                $data = array_merge($data, $this->mapParticipant($participant));
            }
     ...
     * @return array
     */
    public function mapParticipant(Participant $participant)
    {
        $data = array();

        foreach ($participant->users as $user) {
            $data = array_merge($data, $this->mapUser($user));
        }
    ...
    public function mapToIndex()
    {
        $data = array_merge(
            $this->mapData(),
            $this->mapExecutionTimesAndAssignements(),
    ...
    private static function getExecutionTimesMap()
    {
        return array(
            'expiration' => 'sensor_expiration_dt',
            'expiration_days' => 'sensor_expiration_days_i',
    ...
    private static function patchSearchIndex($postData, $commit = true)
    {
...
//        $this->setPostUserUnaware();
        ...
        foreach ($languages as $languageCode) {
            $collectedData[$languageCode] = [
                ...
                'meta_modified_dt' => \ezfSolrDocumentFieldBase::convertTimestampToDate($this->post->modified->format('U')),
            ];
